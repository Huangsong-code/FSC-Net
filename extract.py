...
from model_809 import embed_net
from utils import *
import pdb
import scipy.io

parser = argparse.ArgumentParser(description='PyTorch Cross-Modality Training')
parser.add_argument('--dataset', default='llcm', help='dataset name: llcm or sysu]')
parser.add_argument('--lr', default=0.1 , type=float, help='learning rate, 0.00035 for adam')
parser.add_argument('--optim', default='sgd', type=str, help='optimizer')
parser.add_argument('--arch', default='resnet50', type=str, help='network baseline:resnet18 or resnet50')
parser.add_argument('--resume', '-r', default='llcm_deen_p4_n8_lr_0.1_seed_0_best.t', type=str, help='resume from checkpoint')
parser.add_argument('--test-only', action='store_true', help='test only')
parser.add_argument('--model_path', default='/home/FD_809/save_model/', type=str, help='model save path')
parser.add_argument('--save_epoch', default=20, type=int, metavar='s', help='save model every 10 epochs')
parser.add_argument('--log_path', default='/home/FD_809/log/', type=str, help='log save path')
parser.add_argument('--vis_log_path', default='/home/FD_809/log/vis_log/', type=str, help='log save path')
parser.add_argument('--workers', default=4, type=int, metavar='N', help='number of data loading workers (default: 4)')
parser.add_argument('--img_w', default=144, type=int, metavar='imgw', help='img width')
parser.add_argument('--img_h', default=388, type=int, metavar='imgh', help='img height')
...
parser.add_argument('--ablation', default='fgs', type=str, help='all for total, fsa for only FSA, gca for only GCA,  st for only ST')

args = parser.parse_args()
os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu

method = 'Initial'
if not os.path.isdir(f'/home/{method}/'):
    os.makedirs(f'/home/{method}/')
...
if dataset == 'sysu':
    ...
    data_path = '/root/autodl-tmp/LLCM/'
    n_class = 713
...
net = embed_net(n_class, dataset, arch=args.arch, ablation=args.ablation)
net.to(device)    
...
def extract_gall_feat(gall_loader):
    ...
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(gall_loader):
            ...
            gall_feat_pool[ptr:ptr+batch_num,: ] = feat_pool[:feat_pool.size(0)//3].detach().cpu().numpy()
            gall_feat_fc[ptr:ptr+batch_num,: ]   = feat_fc[:feat_pool.size(0)//3].detach().cpu().numpy()
            ptr = ptr + batch_num
...
def extract_query_feat(query_loader):
    ...
    with torch.no_grad():
        for batch_idx, (input, label) in enumerate(query_loader):
            ...
            query_feat_pool[ptr:ptr+batch_num,: ] = feat_pool[:feat_pool.size(0)//3].detach().cpu().numpy()
            query_feat_fc[ptr:ptr+batch_num,: ]   = feat_fc[:feat_pool.size(0)//3].detach().cpu().numpy()
            ptr = ptr + batch_num         
    print('Extracting Time:\t {:.3f}'.format(time.time()-start))
...
if dataset == 'llcm':
    ...
    scipy.io.savemat(f'/home/{method}/tsne.mat',result)
